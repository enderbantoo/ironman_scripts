 ...
 * quests-csv-to-json
 *
 * Reads a quest CSV file and outputs Quest objects as JSON.
 *
 * Usage:
 *   tsx src/quests-csv-to-json.ts [options] [input.csv]
 *
 * Options:
...
function main() {
  ...
  const quests = questsFromCsv(raw, maps);
  ...
  if (opts.outputDir) {
    // One file per quest, named <questId>.json
    try {
    ...
    process.stderr.write(`Wrote ${quests.length} quest files to ${opts.outputDir}/\n`);
  } else if (opts.output) {
    // All quests as a JSON array in a single file
    writeFileSync(opts.output, serialize(quests, opts.pretty) + "\n", "utf-8");
    process.stderr.write(`Wrote ${quests.length} quests to ${opts.output}\n`);
  } else {
    // Array to stdout
